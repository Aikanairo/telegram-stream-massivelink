...
class ByteStreamer:
    ...
    async def get_file_properties(self, message_id: int, chid = None) -> FileId:
        """
        ...
        if message_id not in self.cached_file_ids:
            await self.generate_file_properties(message_id, chid)
            logger.debug(f"Cached file properties for message with ID {message_id}")
        return self.cached_file_ids[message_id]
    
    async def generate_file_properties(self, message_id: int, chid = None ) -> FileId:
        """
        ...
        if chid != None:
            channel_id = int(chid)
            print(channel_id)
        file_id = await get_file_ids(self.client, channel_id, message_id)
